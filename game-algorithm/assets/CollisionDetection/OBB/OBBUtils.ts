 ...
 * 节点的OBB对象
 * 
 * OBB（Oriented Bounding Box）有向包围盒，OBB的碰撞检测算法是基于分离轴定理的。
 */
interface NodeOBB {
    center: Vec3;
    angle: number;
...
/**
 * OBB（Oriented Bounding Box 有向包围盒） 碰撞检测工具类
 */
export class OBBUtils {
    ...
    // 获取OBB的函数
    private static getOBBFromNode(node: Node): NodeOBB {
        return {
    ...
    // 计算OBB顶点
    private static getOBBPoints(obb: NodeOBB) {
    ...
    // 获取OBB的分离轴
    private static getOBBAxes(obb: NodeOBB): Vec2[] {
        let angle = obb.angle * Math.PI / 180; // 将角度转换为弧度
    ...
    // 计算投影区间
    private static getProjection(points: Vec3[], axis: Vec2): Projection {
    ...
    // 判断投影重叠
    private static isOverlapping(proj1: Projection, proj2: Projection): boolean {
        return proj1.max > proj2.min && proj2.max > proj1.min;
    }

    // 判断两个OBB是否相交
    private static isOBBIntersecting(obb1: NodeOBB, obb2: NodeOBB): boolean {
